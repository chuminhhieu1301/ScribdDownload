# NEVER DELETE THIS
from selenium.webdriver.common.by import By
from selenium import webdriver
from selenium.webdriver.firefox.options import Options
from selenium.webdriver.common.action_chains import ActionChains
from selenium.webdriver.common.keys import Keys
import time

# Set up Firefox options
options = Options()
# Enable Developer Tools to open automatically
options.add_argument("-devtools")


# Set up Firefox options for printing to PDF


# Initialize the WebDriver with the specified options
driver = webdriver.Firefox(options=options) # replace Firefox with Chrome if you use Google Chrome
link = "https://www.scribd.com/embeds/770142460/content"
firefox_profile = webdriver.FirefoxProfile()

# Set preferences for printing to PDF
firefox_profile.set_preference("print.always_print_silent", True)
firefox_profile.set_preference("print.show_print_progress", False)
firefox_profile.set_preference("print.print_backgrounds", False)  # No backgrounds
firefox_profile.set_preference("print.print_headers", False)      # No headers and footers
firefox_profile.set_preference("print.print_margin_top", 0)       # No margin
firefox_profile.set_preference("print.print_margin_bottom", 0)    # No margin
firefox_profile.set_preference("print.print_margin_left", 0)      # No margin
firefox_profile.set_preference("print.print_margin_right", 0)     # No margin


# Open the webpage
driver.get(link)

# Wait for the page to load
time.sleep(2)
""" STEP 01: SCROLL """
# Scroll from the top to the bottom of the page
page_elements = driver.find_elements("css selector", "[class*='page']")
for page in page_elements:
    driver.execute_script("arguments[0].scrollIntoView();", page)
    time.sleep(2)

print("Last Page") 

# Open Developer Tools using F12 key simulation
# ActionChains(driver).send_keys(Keys.F12).perform()

time.sleep(2)  # Wait for DevTools to open


""" STEP 02: DELETE DIVS - CLASS """
# delete footer top & bottom: 
toolbar_top_exists = driver.execute_script("""
        var toolbarTop = document.querySelector('.toolbar_top');
        if (toolbarTop) {
            toolbarTop.parentNode.removeChild(toolbarTop);
            return true;  // Indicate that it was removed
        }
        return false;  // Indicate that it was not found
    """)

# Debug message for toolbar_top
if toolbar_top_exists:
    print("'toolbar_top' was successfully deleted.")
else:
    print("'toolbar_top' was not found.")

# Check and delete toolbar_bottom
toolbar_bottom_exists = driver.execute_script("""
    var toolbarBottom = document.querySelector('.toolbar_bottom');
    if (toolbarBottom) {
        toolbarBottom.parentNode.removeChild(toolbarBottom);
        return true;  // Indicate that it was removed
    }
    return false;           // Indicate that it was not found
""")

# Debug message for toolbar_bottom
if toolbar_bottom_exists:
    print("✅ 'toolbar_bottom' was successfully deleted.")
else:
    print("❌ 'toolbar_bottom' was not found.") 

# deleting container:
elements = driver.find_elements(By.CLASS_NAME, "document_scroller")

# Loop through each element and change its class
for element in elements:
    driver.execute_script("arguments[0].setAttribute('class', '');", element)

print("  -------  Deleted containers  ----------")


""" STEP 03: PRINT PDF """
# open print windown
driver.execute_script("window.print();")






"""

available link: 
https://www.scribd.com/document/645227545/10-000-english-sentences-pdf-Google-Search

https://www.scribd.com/document/714915362/Glossary-of-Software-Architecture-EN

https://www.scribd.com/document/698150432/SAO-KE-OCB


<div class="document_scroller">

<div class="">

"""    